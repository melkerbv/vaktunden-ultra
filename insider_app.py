...
def save_user_tickers(u, tickers):
    ...
    for t in tickers: c.execute("INSERT INTO user_tickers VALUES (?, ?)", (u, t))
    conn.commit(); conn.close()
...
def hämta_insider_data(dagar):
    try:
        ...
        def cln(v):
            ...
            return float(s_clean.replace(',', '.')) if s_clean else 0
        df['Kurs_Num'] = df['Pris'].apply(cln); df['Volym_Num'] = df['Volym'].apply(cln)
        df['Kurs'] = df['Kurs_Num'].apply(lambda x: f"{x:,.2f} kr".replace(',', ' ')); df['Värde'] = (df['Volym_Num'] * df['Kurs_Num']).apply(lambda x: f"{x:,.0f} kr".replace(',', ' '))
        return df.sort_values('Publiceringsdatum', ascending=False)
    except: return pd.DataFrame()

if 'auth' not in st.session_state: st.session_state.auth = {'in': False, 'user': None, 'role': 'user', 'prem': False}
    ...
    if not st.session_state.auth['in']:
        ...
        if action == "Skapa konto":
            ...
            if st.button("Skapa Konto", type="primary") and u and em and p1:
                conn = sqlite3.connect('vakthunden.db'); c = conn.cursor()
                if c.execute("SELECT username FROM users WHERE username = ?", (u,)).fetchone(): st.error("Namnet är taget.")
                else:
                    role = 'admin' if c.execute("SELECT COUNT(*) FROM users").fetchone()[0] == 0 else 'user'
                    c.execute("INSERT INTO users(username, email, password, role) VALUES (?,?,?,?)", (u, em, make_hashes(p1), role)); conn.commit()
                    st.session_state.auth = {'in': True, 'user': u, 'role': role, 'prem': False}; st.rerun()
                conn.close()
        else:
            p = st.text_input("Lösenord", type="password")
            if st.button("Logga in", type="primary"):
                conn = sqlite3.connect('vakthunden.db'); c = conn.cursor()
                data = c.execute("SELECT password, role, is_premium FROM users WHERE username =?", (u,)).fetchone(); conn.close()
                if data and check_hashes(p, data[0]): st.session_state.auth = {'in': True, 'user': u, 'role': data[1], 'prem': bool(data[2])}; st.rerun()
                else: st.error("Fel inloggning.")
    else:
        st.success(f"Inloggad: {st.session_state.auth['user']}")
        if st.button("Logga ut"): st.session_state.auth = {'in': False, 'user': None, 'role': 'user', 'prem': False}; st.rerun()
...
if st.session_state.auth['in']:
    ...
    with tabs[0]:
        ...
        with st.spinner("Hämtar live-kurser..."): m_data = get_market_overview()
        c1, c2, c3, c4 = st.columns(4)
        def dm(col, t, d, p=""): col.metric(t, f"{p}{d['val']:,.2f}", f"{d['pct']:+.2f}%") if d else col.metric(t, "N/A", "N/A")
        dm(c1, "OMXS30", m_data.get("OMXS30")); dm(c2, "NASDAQ", m_data.get("NASDAQ")); dm(c3, "S&P 500", m_data.get("S&P 500")); dm(c4, "Bitcoin", m_data.get("Bitcoin"), "$")
        st.write("---")
        ...
        with st.spinner("Laddar nyheter..."):
            ...
            else: st.info("Inga nyheter just nu.")
    ...
    with tabs[1]:
        ...
        saved = load_user_tickers(st.session_state.auth['user']); valda = st.multiselect("Välj tillgångar:", list(set(std + saved)), saved if saved else ["BTC-USD"])
        ny_t = st.text_input("Lägg till egen Ticker:").upper().strip()
        if st.button("Lägg till") and ny_t and ny_t not in valda: valda.append(ny_t); save_user_tickers(st.session_state.auth['user'], valda); st.rerun()
        c1, c2 = st.columns(2)
        if c1.button("Spara lista"): save_user_tickers(st.session_state.auth['user'], valda); st.toast("Sparat!")
        if c2.button("🚀 Kör full scan", type="primary"):
            ...
            st.dataframe(pd.DataFrame(res).sort_values("RSI"), use_container_width=True, hide_index=True) if res else st.warning("Rate limit blockad.")
    ...
    with tabs[2]:
        ...
        with c_l:
            ...
            st.dataframe(df_i[['Publiceringsdatum', 'Emittent', 'Person i ledande ställning', 'Kurs', 'Värde']].head(30) if not df_i.empty else pd.DataFrame(), use_container_width=True, hide_index=True) if not df_i.empty else st.info("Ingen data.")
        with c_r:
            ...
            if s:
                try:
                    ...
                    if not h.empty:
                        fig = go.Figure(data=[go.Scatter(x=h.index, y=h['Close'])]); fig.update_layout(template="simple_white", height=350, margin=dict(l=0,r=0,t=0,b=0)); st.plotly_chart(fig, use_container_width=True)
                    else: st.warning("Hittar ej data.")
                except: st.warning("Rate Limit.")
    ...
    with tabs[3]:
        if st.session_state.auth['role'] == 'admin':
            ...
            if st.button("GE PREMIUM"): conn.execute("UPDATE users SET is_premium = 1 WHERE username = ?", (t,)); conn.commit(); st.success(f"{t} uppgraderad!"); st.rerun()
            conn.close()
        else: st.warning("Åtkomst nekad.")
else:
    ...
    c1.subheader("🎯 Algoritmisk RSI"); c1.write("Skanna marknaden i realtid efter överköpta och översålda lägen.")
    c2.subheader("📊 Insider Flow"); c2.write("Följ smarta pengar direkt från Finansinspektionen.")
    c3.subheader("⚡ Web3-redo"); c3.write("Förbered din portfölj för decentraliserad finansiering.")
